...
class ScheduleImageRenderer:
    ...
    def _render_schedule_image_local(
        ...
        footer_text = "爱驼推送 / Generated by LEN"
    ...
    def _wrap_schedule_items(
        ...
        for item in items:
            ...
            row_height = max(150, 94 + content_height)
    ...
    def _draw_schedule_row(
        ...
        highlight_palette = SCHEDULE_HIGHLIGHT_PALETTES.get(
            ...
            SCHEDULE_HIGHLIGHT_PALETTES["platinum"],
        )
        if item.highlighted:
            draw.rounded_rectangle(
                (list_left - 7, row_y - 7, list_right + 7, row_bottom + 7),
                radius=32,
                fill=highlight_palette["glow"],
            )
            draw.rounded_rectangle(
                (list_left - 3, row_y - 3, list_right + 3, row_bottom + 3),
                radius=29,
                outline=highlight_palette["soft_outline"],
                width=5,
            )
        draw.rounded_rectangle(
            (list_left, row_y, list_right, row_bottom),
            radius=26,
            fill=highlight_palette["fill"] if item.highlighted else "#fffaf4",
            outline=highlight_palette["outline"] if item.highlighted else "#eadbc9",
            width=4 if item.highlighted else 2,
        )
        draw.rounded_rectangle(
            (list_left + 24, row_y + 22, list_left + 168, row_y + row_height - 22),
            radius=24,
            fill=highlight_palette["time_fill"] if item.highlighted else "#f1e4d3",
        )
